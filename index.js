...
async function main() {
  const parsingRegex = /(\d+)-(\d+) (\w): (\w+)/;
  let passingPasswords = 0;
  let failingPasswords = 0;
  for (const pw of input) {
    const result = parsingRegex.exec(pw);
    const firstLetterPos = result[1];
    const secondLetterPos = result[2];
    const policyLetter = result[3];
    const password = result[4];
    
    const firstLetter = password[firstLetterPos - 1];
    const secondLetter = password[secondLetterPos - 1];

    if (firstLetter === policyLetter) {
      if (secondLetter === policyLetter) {
        failingPasswords += 1;
        continue;
      }
      passingPasswords += 1;
  ...
  console.log(`Passing ${passingPasswords}`);
